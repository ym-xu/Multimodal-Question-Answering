...
def load_pop_data(data_path=None, global_rank=-1, world_size=-1, poplevel=0, popdata=None):
    assert data_path
    if data_path.endswith('.jsonl'):
        data = open(data_path, 'r')
    elif data_path.endswith('.json'):
    ...
    for k, example in enumerate(data):
        ...
        #if max_ == poplevel:
        #    examples.append(example)
        if len(example['subj']) == 0:
            continue
        if example['subj'] in poplevels.keys():
            if poplevels[example['subj']] == poplevel:
                examples.append(example)
    ## egrave: is this needed?
    if data_path is not None and data_path.endswith('.jsonl'):
        data.close()
