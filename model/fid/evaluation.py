...
####################################################
########    CANDIDATE OBJECTS EVALUATION    ########
####################################################
def normalize_answer_cobjs(s):
    def remove_articles_cobjs(text):
        return regex.sub(r'\b(a|an|the)\b', ' ', text)

    def white_space_fix_cobjs(text):
        return ' '.join(text.split())

    def remove_punc_cobjs(text):
        exclude = set(string.punctuation)
        return ''.join(ch for ch in text if ch not in exclude)

    def lower_cobjs(text):
        return text.lower()

    return white_space_fix_cobjs(remove_articles_cobjs(remove_punc_cobjs(lower_cobjs(s))))

def exact_match_score_cobjs(prediction, ground_truth):
    return normalize_answer_cobjs(prediction) == normalize_answer_cobjs(ground_truth)

def ems_cobjs(prediction, ground_truths):
    max_score = 0
    
    for pred_val in prediction:
        for gt_val in ground_truths:
            score = exact_match_score_cobjs(pred_val, gt_val)  # 假设 exact_match_score 是一个计算精确匹配分数的函数
            if score > max_score:
                max_score = score
